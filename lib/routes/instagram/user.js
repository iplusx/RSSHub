    ...
    const media = await Promise.all(
        list.map(async (item) => {
            ...
            if (item.__typename === 'GraphImage') {
                ...
                const data = JSON.parse(response.data.match(/<script type="text\/javascript">window._sharedData = (.*);<\/script>/)[1]) || {};
                const single = {
                    poster: data.entry_data.PostPage[0].graphql.shortcode_media.display_url,
        ...
        item: list.map((item, index) => {
            ...
            for (let i = 0; i < media[index].length; i++) {
                ...
                if (media[index][i].video) {
                    content += `<video width="100%" controls="controls" src="${media[index][i].video}" poster="${media[index][i].poster}" type="video/mp4"></video>`;
                    video++;
                }
            }
